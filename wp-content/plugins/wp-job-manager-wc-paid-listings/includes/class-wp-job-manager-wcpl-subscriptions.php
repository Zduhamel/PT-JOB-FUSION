...
/**
 * WP_Job_Manager_WCPL_Subsriptions
 */
class WP_Job_Manager_WCPL_Subsriptions {
	...
	public function subscription_ended( $user_id, $subscription_key ) {
		...
		$user_packages = wc_paid_listings_get_user_packages( $user_id );

		foreach ( $user_packages as $user_package ) {
			// See if this package matches subscription
			if ( $user_package->order_id == $subscription['order_id'] && $user_package->product_id == $subscription['product_id'] ) {
	...
	public function switched_subscription( $user_id, $original_subscription_key, $new_subscription_key ) {
		...
		// Now loop and find old package
		foreach ( $user_packages as $user_package ) {
			// See if this package matches subscription
			if ( $user_package->order_id == $old_subscription['order_id'] && $user_package->product_id == $old_subscription['product_id'] ) {
...
new WP_Job_Manager_WCPL_Subsriptions();