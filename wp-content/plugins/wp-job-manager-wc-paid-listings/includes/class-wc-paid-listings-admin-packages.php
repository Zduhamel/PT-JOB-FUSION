...
class WP_Job_Manager_WCPL_Admin_Packages extends WP_List_Table {
	...
	public function column_default( $item, $column_name ) {
		...
		switch( $column_name ) {
			...
			case 'limit' :
				return $item->package_limit ? sprintf( __( '%s Posted', 'wp-job-manager-wc-paid-listings' ), absint( $item->package_count ) . ' / ' . absint( $item->package_limit ) ) : __( 'Unlimited', 'wp-job-manager-wc-paid-listings' );
			case 'package_type' :
